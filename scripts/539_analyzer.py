...
def generate_json(latest_draw, selected_nums, threshold, top_recommended, scores, drag_freq, gap_stats, hot_nums, cold_nums, matches, hot_cold_counts, backtest_data):
    ...
    try:
        ...
        # 6. 自動更新 sw.js 裡面的快取名稱，以解決 PWA 頑固緩存的問題
        sw_path = os.path.join(webapp_dir, "sw.js")
        if os.path.exists(sw_path):
            try:
                ...
                new_cache_name = f"const CACHE_NAME = '539-analysis-cache-{timestamp}';"
                ...
                with open(sw_path, 'w', encoding='utf-8') as f:
                    f.write(updated_sw)
                print(f"成功更新 sw.js 的快取版本號為: {timestamp}")
            except Exception as sw_err:
                print(f"警告: 自動更新 sw.js 失敗: {sw_err}")
